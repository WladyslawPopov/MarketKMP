...
fun ProposalsItemContent(
    ...
    goToUser: (Long) -> Unit,
    confirmProposal: () -> Unit
) {
    val user = offer.sellerData
            ...
            proposals.proposals.forEach { proposal ->
                ...
                when (proposal.clas) {
                    "proposal_accept" -> {
                        ...
                        typeLabel.value = stringResource(
                            if (!proposal.isResponserProposal) {
                                strings.proposalAcceptLabel
                            } else {
                                strings.sellerProposalAcceptLabel
                            }
                        )
                    }

                    "proposal_unanswered" -> {
                        showEnd.value = proposal.isResponserProposal
                        showBody.value = proposal.isResponserProposal
                    ...
                    "proposal_reject" -> {
                        showEnd.value = false
                        showBody.value =
                            (proposal.createdTs == proposals.proposals.lastOrNull()?.createdTs &&
                ...
                Column(
                    ...
                    Column(
                        ...
                        val proposalLabel = buildAnnotatedString {
                            if (!proposal.isResponserProposal) {
                                append(stringResource(strings.yourProposeLabel))
                            } else {
                                append(stringResource(strings.sellerProposeLabel))
                            }
                            append(" ")
                            withStyle(
                                SpanStyle(
                            ...
                            withStyle(
                                ...
                                append(proposal.quantity.toString())
                            }
                            append(" ")
                        ...
                        if (showBody.value) {
                            getBody(offer.id, type, viewModel) {
                                confirmProposal()
                            }
                        }
                    }
...
fun getBody(
    ...
    val fields = fieldsState.value?.fields

    LaunchedEffect(Unit){
        viewModel.getFieldsProposal(offerId, proposalType)
    }
            ...
            item {
                Row(
                    ...
                    SimpleTextButton(
                        ...
                        if (proposalType == ProposalType.MAKE_PROPOSAL)
                            fields.find { it.key == "type" }?.data = JsonPrimitive(2)
