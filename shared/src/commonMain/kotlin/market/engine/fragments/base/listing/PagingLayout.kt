...
fun <T : Any> PagingLayout(
    ...
    val listingDataState = viewModel.listingData.collectAsState()
    ...
    val columns = remember(listingType.value) {  if (listingType.value == 0) 1 else 2 }
    ...
    val align = remember { if (isReversingPaging.value) Alignment.BottomStart else Alignment.TopStart }
    ...
    Box(
        ...
        {
            if (promoList.value.isNotEmpty() && promoContent != null) {
                item {
            ...
            { index ->
                if (promoContent != null && !searchData.userSearch && searchData.searchString.isEmpty()) {
                    if (index > 0) {
